...
class Ccavenue_model extends CI_Model 
{
	...
	public function set_ccavenue_form_submit(){
			...
			$this->ccavenue_class->add_field('cancel_url', $this->general->lang_uri('/'.MY_ACCOUNT.'/purchase/cancel'));
	...
	public function get_transaction_data($invoice_id)
	{
	...
	public function update_transaction($invoice_id,$txn_id){
	...
	public function insert_transaction_log($txn_details)
	{
		$data = array(	
			'txn_details'=>$txn_details,			
			'post_date' => $this->general->get_local_time('time')			
			);
		$this->db->insert('txn_log', $data);
		return $this->db->insert_id(); 
	}
