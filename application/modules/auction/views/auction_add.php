...
$(document).ready(function() {
  Dropzone.options.myAwesomeDropzone = {
   maxFiles:  '<?php echo MAXIMUM_NUMBERS_OF_PRODUCT_IMAGES; ?>',
    // maxFiles: 2,
...
//print_r($error);
?>
<form name="sitesetting" id="" method="post" action=""  enctype="multipart/form-data" accept-charset="utf-8">
...
<tr>
  <td width="200" class="hmenu_font">Auction Price (<?php echo DEFAULT_CURRENCY_SIGN;?>)</td>
  <td><input name="price" type="text" class="inputtext" id="price" value="<?php echo set_value('price');?>" size="" />
   <div class="has-error"><?=form_error('price')?></div>

   </td>
  </tr>
<tr>
  <td class="hmenu_font">Shipping Cost (<?php echo DEFAULT_CURRENCY_SIGN;?>)</td>
  <td><input name="shipping_cost" type="text" class="inputtext" id="shipping_cost" value="<?php echo set_value('shipping_cost');?>" size="15" />
    <?=form_error('shipping_cost')?></td>
</tr>
<tr>
  <td class="hmenu_font">Bid Fee </td>
  <td><input name="bid_fee" type="text" class="inputtext" id="bid_fee" value="<?php echo set_value('bid_fee');?>" size="10" />
  Bid(s)
    <?=form_error('bid_fee')?></td>
</tr>
<!-- <tr>
  <td class="hmenu_font">Minimum Bids Value</td>
  <td>
  <input name="min_bids_value" type="text" class="inputtext" id="min_bids_value" value="<?php echo set_value('min_min_bids_value');?>" size="10" />
    <?=form_error('min_min_bids_value')?></td>
</tr>
 -->
<tr>
  <td class="hmenu_font">SMS Code</td>
  <td>
  <input name="sms_code" type="text" class="inputtext" id="sms_code" value="<?php echo set_value('sms_code');?>" size="10" />
    <?=form_error('sms_code')?></td>
</tr>

<!-- <tr>
  <td class="hmenu_font">Auction Reset Time</td>
  <td><input name="reset_time" type=text class="inputtext" id="reset_time" value="<?php echo set_value('reset_time');?>" size="10"> Sec
<?=form_error('reset_time')?></td>
</tr> -->
<tr>
  <td class="hmenu_font">Start Date </td>
  <td>
  <input name="start_date" type="text" class="inputtext" id="start_date" value="<?php echo set_value('start_date');?>" size="20" readonly="readonly" />
...
<tr>
  ...
  <td>
  <input name="end_date" type="text" class="inputtext" id="end_date" value="<?php echo $this->input->post('end_date');?>" size="20" />
...
<td width="429">
       ...
       <a href="javascript:void(0);" class="dz-remove remove_image" data-job="<?php echo $jobs; ?>" data-imgname="<?php echo $image->image; ?>" data-imgid="<?php echo $random; ?>" data-pid='<?php echo ($jobs=='edit')?$product->id:''; ?>'  data-pcode="<?php echo ($jobs=='relist')?$product_code:''; ?>">Remove file</a> </div>
                  <?php
                  }
                }
              ?>
...
<tr>
  ...
  <td>
  <input name="is_featured" type="radio" value="Yes" <?php if(isset($_POST['is_featured']) && $_POST['is_featured'] == 'Yes'){ echo 'checked="checked"';}?>/>
    Yes
      <input name="is_featured" type="radio" value="No" checked="checked" />
      No  </td>
</tr>
...
<tr>
  ...
  <input name="is_display" type="radio" value="Yes" checked="checked" />
    Yes
      <input name="is_display" type="radio" value="No"  <?php if(isset($_POST['is_display']) && $_POST['is_display'] == 'No'){ echo 'checked="checked"';}?>  />
      No  </td>
</tr>
...
<!-- <tr>
  ...
  <input name="is_bidbutler" type="radio" value="Yes" checked="checked" />
    Yes
      <input name="is_bidbutler" type="radio" value="No" <?php if(isset($_POST['is_bidbutler']) && $_POST['is_bidbutler'] == 'No'){ echo 'checked="checked"';}?> />
      No  </td>
</tr>
...
<tr>
  ...
  <td>
  <input name="is_bid_package" type="radio" value="Yes"  <?php if(isset($_POST['is_bid_package']) && $_POST['is_bid_package'] == 'Yes'){ echo 'checked="checked"';}?> />
    Yes
      <input name="is_bid_package" type="radio" value="No" <?php if(empty($_POST['is_bid_package']) || $_POST['is_bid_package'] == 'No'){ echo 'checked="checked"';}?> />
      No  </td>
</tr>
 -->
<tr>
  ...
  <input name="is_buy_now" type="radio" value="Yes" onclick="show_buy_now()"  <?php if(isset($_POST['is_buy_now']) && $_POST['is_buy_now'] == 'Yes'){ echo 'checked="checked"';}?> />
    Yes
      <input name="is_buy_now" type="radio" value="No"  onclick="hide_buy_now()" <?php if(empty($_POST['is_buy_now']) || $_POST['is_buy_now'] == 'No'){ echo 'checked="checked"';}?> />
      No  </td>
</tr>

<tr id="buy_now_qty" <?php if(isset($_POST['is_buy_now']) && $_POST['is_buy_now'] == 'Yes'){ echo '';}else{echo 'style="display:none;"';}?>>
  <td class="hmenu_font">Buy Now Quantity</td>
  <td>
  <input name="no_qty" type="text" class="inputtext" id="no_qty" value="<?php echo set_value('no_qty');?>" size="10" />
  </td>
</tr>
...
<tr height="30">
  <td colspan="2"><div style=" background-color:#FFFFFF; padding:5px 10px;"><strong>Auction Details Based on Language </strong></div></td>
</tr>
<tr>
  <td class="hmenu_font">Choose Language to Post Auction in </td>
  <td>
  ...
  <?php /*?><input type="checkbox" name="lang[<?php echo 'lang_id_'.$lang_details[$i-1]->id;?>]" value="<?php echo $lang_details[$i-1]->id;?>" <?php if($language_id == $lang_details[$i-1]->id) echo 'checked="checked"'; ?> /><?php echo $lang_details[$i-1]->lang_name;?> <?php */?>
   ...
   <input type="checkbox" class="checklang" id="<?php echo 'formelement'.$lang_details[$i]->id?>" name="lang_id[]" value="<?php echo $lang_details[$i]->id;?>" <?php if($language_id == $lang_details[$i]->id) echo 'checked="checked"'; ?> /><?php echo $lang_details[$i]->lang_name;?> 
...
<tr>
      <td width="660"><strong>Auction Name <?php if(count($lang_details)!=1){?>(<?php echo $lang_details[$i-1]->lang_name;?>)<?php }?> </strong></td>
</tr>

<tr>
<td>
<?php $auc_name = ''; $name = $this->input->post('name'); if(!empty($name[$lang_details[$i-1]->id])){ $auc_name = $name[$lang_details[$i-1]->id]; };?>
<input size="50" data-tabid="<?php  echo $i;?>"  class="inputtext <?php echo 'formelement'.$lang_details[$i-1]->id.'_input'?>" type="text" id="name[<?php echo $lang_details[$i-1]->id;?>]" name="name[<?php echo $lang_details[$i-1]->id;?>]" value="<?php echo $auc_name; ?>">
<?php //echo '<div class="error">'.$this->session->userdata('name_'.$lang_details[$i-1]->id)."</div>"; ?></td>
</tr>


<tr>
      <td><strong>Acution Description <?php if(count($lang_details)!=1){?>(<?php echo $lang_details[$i-1]->lang_name;?>)<?php }?></strong></td>
</tr>
...
<td>
		...
		<?php echo '<div class="error">'.$this->session->userdata('description_'.$lang_details[$i-1]->id)."</div>"; ?>		</td>
</tr>
...
<tr>
      <td width="660"><strong>Page Title <?php if(count($lang_details)!=1){?>(<?php echo $lang_details[$i-1]->lang_name;?>)<?php }?></strong></td>
</tr>

<tr>
<td>
<?php $page_title = ''; $page_title2 = $this->input->post('page_title'); if(!empty($page_title2[$lang_details[$i-1]->id])){ $page_title = $page_title2[$lang_details[$i-1]->id]; };?>
<textarea cols="60" rows="1" id="page_title[<?php echo $lang_details[$i-1]->id;?>]" name="page_title[<?php echo $lang_details[$i-1]->id;?>]"><?php echo $page_title;?></textarea></td>
</tr>
<tr>
      <td width="660"><strong>Meta Keys <?php if(count($lang_details)!=1){?>(<?php echo $lang_details[$i-1]->lang_name;?>)<?php }?></strong></td>
</tr>

<tr>
<td>
<?php $meta_key = ''; $meta_key2 = $this->input->post('page_title'); if(!empty($meta_key2[$lang_details[$i-1]->id])){ $meta_key = $meta_key2[$lang_details[$i-1]->id]; };?>
<textarea cols="60" rows="2" id="meta_key[<?php echo $lang_details[$i-1]->id;?>]" name="meta_key[<?php echo $lang_details[$i-1]->id;?>]"><?php echo $meta_key;?></textarea></td>
</tr>



<tr>
      <td width="660"><strong>Meta Description <?php if(count($lang_details)!=1){?>(<?php echo $lang_details[$i-1]->lang_name;?>) <?php }?></strong></td>
</tr>

<tr>
<td>
<?php $meta_desc = ''; $meta_desc2 = $this->input->post('page_title'); if(!empty($meta_desc2[$lang_details[$i-1]->id])){ $meta_desc = $meta_desc2[$lang_details[$i-1]->id]; };?>
<textarea cols="60" rows="2" id="meta_desc[<?php echo $lang_details[$i-1]->id;?>]" name="meta_desc[<?php echo $lang_details[$i-1]->id;?>]"><?php echo $meta_desc;?></textarea></td>
</tr>
...
<tr height="30">
  ...
  <td>&nbsp;</td>
  <td colspan="2"><input class="bttn" type="submit" id="" name="Submit" value="Submit" /></td>
</tr>
</table>
</form>
