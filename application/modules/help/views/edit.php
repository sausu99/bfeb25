...
<tr>
  <td class="hmenu_font">Choose Help Category </td>
  <td>
  <?php if($this->input->post('help_category'))$help_category_id=$this->input->post('help_category'); else $help_category_id = $data_help->help_cat_id; ?>
  <select name="help_category">
  <option value="" selected="selected">Choose Help Category</option>
  <?php foreach($this->admin_help->get_help_category_byid($lang_id) as $help_cat){?>
  <option value="<?php echo $help_cat->id;?>" <?php if($help_category_id == $help_cat->id ) {echo 'selected="selected"';} ?>><?php echo $help_cat->help_category_name;?></option>
  <?php }?>
  </select>  </td>
</tr>
<tr>
  <td width="229" class="hmenu_font">Help Title </td>
