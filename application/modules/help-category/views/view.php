...
<div style="padding:6px 20px; float:left; width:610px; background:#202020; color:#ABABAB;"><span class="breed"><a href="<?=site_url(ADMIN_DASHBOARD_PATH)?>">ADMIN</a> &raquo; Help Category Management </span></div>
	...
	<h2>View Help Category Details </h2>
    <div class="mid_frm">
      ...
      <table width="100%">
        <tr>
          <td>
		  <ul id="vList">
		   <li>[ <strong><a href="<?php echo site_url(ADMIN_DASHBOARD_PATH);?>/help-category/add_help_category">Add Help Category</a></strong> ]</li>
		  <?php
		  	 if(count($lang_details) !=1){
		  		foreach($lang_details as $lang)
				{
		?>
		  <li>[ <strong><?php if($lang_id != $lang->id){?><a href="<?php echo site_url(ADMIN_DASHBOARD_PATH);?>/help-category/index/<?php echo $lang->id;?>"><?php }?><?php echo $lang->lang_name;?></a></strong> ]</li>
		 <?php
		 		}}
		?>
		  </ul>
...
<table width="100%" border="0" cellspacing="0" cellpadding="4" class="contentbl">
  ...
  <th width="40%" align="left">Name</th>
	...
	?>
  <tr> 
    <td align="left"><?php print($data->help_category_name);?></td>
	...
	<a href="<?php echo site_url(ADMIN_DASHBOARD_PATH);?>/help-category/edit_help_category/<?php print($lang_id);?>/<?php print($data->id);?>" style="margin-right:5px;">
      <img src='<?php print(ADMIN_IMG_DIR_FULL_PATH);?>/edit.gif' title="Edit"></a>   <a  style="margin-left:5px;" href="<?php echo site_url(ADMIN_DASHBOARD_PATH);?>/help-category/delete_help_category/<?php print($lang_id);?>/<?php print($data->id);?>">
