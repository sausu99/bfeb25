...
class Admin extends CI_Controller {
	...
	public function add_banner()
	{
		...
		{			
			...
			redirect(ADMIN_DASHBOARD_PATH.'/how-it-works/index/'.$this->input->post('lang', TRUE),'refresh');
			exit;
		}
	...
	public function edit_banner($id='')
	{
		...
		if(!isset($id)){redirect(ADMIN_DASHBOARD_PATH.'/how-it-works/index','refresh');exit;}
		...
		if($this->data['result_data'] == false){redirect(ADMIN_DASHBOARD_PATH.'/how-it-works/index','refresh');exit;}
		...
		if(isset($_FILES['banner']['tmp_name']) && $_FILES['banner']['tmp_name']!='')
		{
			...
			$flag_full_path = BANNER_PATH.$image_data['file_name'];
	...
	public function delete_banner($id)
	{
			...
			if($query->num_rows() > 0) 
			{
				...
				$this->db->delete('how_it_works', array('id' => $id));
				$row_data = $query->row();	
				@unlink('./'.$row_data->banner);
				$this->session->set_flashdata('message','The how it record delete successful.');
				redirect(ADMIN_DASHBOARD_PATH.'/how-it-works/index','refresh');
				exit;
			}
			else
				{
					$this->session->set_flashdata('message','The how it record id='.$id.' is not available in our database.');
					redirect(ADMIN_DASHBOARD_PATH.'/how-it-works/index','refresh');
					exit;
				}
	}
