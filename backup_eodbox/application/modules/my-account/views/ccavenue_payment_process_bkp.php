<?php $price=$this->input->post('package_price',true);?>
<script>
  window.onload = function() {
    var d = new Date().getTime();
    document.getElementById("tid").value = d;
    document.getElementById("order_id").value = d;
    $('#display_amount').html($("#amount").val());
...
<table width="50%" height="100" border='1' align="center">
   <input type="hidden" name="tid" id="tid" value="112342134"/>
   <input type="hidden" name="merchant_id" value="131065"/>
   ...
   <input type="hidden" name="transaction_type" value="<?php echo $this->input->post('transaction_type',true);?>">
   ...
   <input type="hidden" id="amount" name="amount" value="<?php if($price)echo $price;else echo set_value('amount');?>"/>
   <tr>
    <td>Amount :</td><td><div id="display_amount"></div></td>
  </tr>
...
<tr> <td> Card Type: </td>
 <td><input type="text" id="card_type" name="card_type" value="" readonly="readonly"/><?php echo form_error('card_type'); ?></td>
</tr>
...
<tr> <td> Data Accepted At </td>
 <td><input type="text" id="data_accept" name="data_accept" readonly="readonly"/><?php echo form_error('data_accept'); ?></td>
</tr>

<tr> <td> Card Number: </td>
  <td> <input type="text" id="card_number" name="card_number" value=""/>e.g. 4111111111111111<?php echo form_error('card_number'); ?> </td>

</tr>

<tr> <td> Expiry Month: </td>
 <td> <input type="text" name="expiry_month" value=""/>e.g. 07<?php echo form_error('expiry_month'); ?> </td>
</tr>

<tr> <td> Expiry Year: </td>
 <td> <input type="text" name="expiry_year" value=""/>e.g. 2027<?php echo form_error('expiry_year'); ?></td>
</tr>

<tr> <td> CVV Number:</td>
 <td> <input type="text" name="cvv_number" value=""/>e.g. 328<?php echo form_error('cvv_number'); ?></td>
</tr>
...
<script>
  ...
  $(function(){
    ...
    */   
    var jsonData;
      var access_code="AVJK70ED32BN82KJNB" // shared by CCAVENUE 
      var amount="<?php echo $this->input->post('package_price',true);?>";
      var currency="INR";

      $.ajax({
       url:'https://test.ccavenue.com/transaction/transaction.do?command=initiateTransaction='+access_code+'&currency='+currency+'&amount='+amount,
       dataType: 'jsonp',
       jsonp: false,
